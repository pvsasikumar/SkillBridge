    ...
    <div className={cn('space-y-2', className)}>
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3 text-sm">
          <span className="font-semibold text-on-surface">{skill.currentLevel}%</span>
          {showGap && gap > 0 && (
            <span className="text-danger-500 font-bold text-xs">-{gap}%</span>
          )}
          <span className="text-outline text-xs">/ {skill.requiredLevel}%</span>
        </div>
      </div>
      <div className="relative h-2 bg-surface-container-highest rounded-full overflow-hidden">
        ...
        {showGap && (
          <div
            className="absolute top-0 h-full bg-outline-light/40 rounded-r-full"
            style={{
  ...
  return (
    <div className={cn('p-4 glass-card space-y-3', className)}>
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs font-medium text-outline">{skill.category}</span>
      </div>
      <SkillBar skill={skill} showGap />
      <div className="flex items-center justify-between pt-1">
        <div className="flex items-center gap-1.5">
          <DotIndicator status={skill.status} />
          <span className="text-xs font-medium text-on-surface-variant capitalize">{skill.status.replace('-', ' ')}</span>
        </div>
        {gap > 0 && (
          <span className="text-xs font-bold text-danger-600">Gap: {gap}%</span>
        )}
      </div>
    </div>
  );
